...
bool win_init_clock(struct libusb_context *ctx)
{
    ...
    if (QueryPerformanceFrequency(&li_frequency)) {
        // Load DLL imports
        if (win_init_dlls() != LIBUSB_SUCCESS) {
        ...
        timer_thread = (HANDLE)_beginthreadex(NULL, 0, win_clock_gettime_threaded, (void *)event,
...
static void win_transfer_callback(struct usbi_transfer *itransfer, uint32_t io_result, uint32_t io_size)
{
    int status, istatus;
